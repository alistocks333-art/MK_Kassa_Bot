...
def fmt(num):
    """Raqamni pul formatiga keltiradi"""
...
def get_worker_filter(uid):
    """Ishchi uchun WHERE sharti va parametrlarni xavfsiz qaytaradi"""
    if uid in BOSS_IDS: return "", ()
    return "AND worker_id = %s", (uid,)

def get_worker_name(uid):
    """Ishchi ismini bazadan xavfsiz olish"""
    try:
...
# ================= BAZA =================
def init_db():
    """Baza jadvallarini yaratish va BIGINT ga o'tkazish"""
    conn = get_db()
    cur = conn.cursor()
    try:
...
def dict_cursor(conn):
    """Lug'at (dict) qaytaruvchi cursor"""
...
# ================= QARZI BORLAR (HAVING TUZATILDI) =================
@dp.message(F.text == "🤝 Qarzi borlar")
async def handle_debtors(message: types.Message):
    uid = message.from_user.id
    conn = get_db(); cur = dict_cursor(conn)
    if uid in BOSS_IDS:
        # ✅ HAVING da alias o'rniga to'liq ifoda ishlatildi
